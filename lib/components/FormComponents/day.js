...
module.exports = (0, _createReactClass2.default)({
  ...
  getDatePart: function getDatePart(config) {
    var classes = 'control-label' + (config.required ? ' field-required' : '');
    ...
    var errorClass = !this.state.isPristine && config.required && !this.state.date[config.key] ? ' has-error' : '';

    return _react2.default.createElement(
      'div',
      { className: 'form-group col-xs-' + config.columns + errorClass },
  ...
  getMonth: function getMonth(componentId, field) {
    ...
    var classes = 'control-label' + (field.required ? ' field-required' : '');
    var options = [field.placeholder, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];

    var errorClass = !this.state.isPristine && field.required && !this.state.date.month ? ' has-error' : '';

    return _react2.default.createElement(
      'div',
      { className: 'form-group col-xs-4' + errorClass },
      _react2.default.createElement(
        'label',
