    ...
    switch (this.props.component.dataSrc) {
      ...
      case 'json':
        try {
          ...
          } else if (_typeof(this.props.component.data.json) === 'object') {
        ...
        this.items.map(function (item) {
          item.render = (0, _util.interpolate)(_this.props.component.template, { item: item }).replace(/<(?:.|\n)*?>/gm, '');
          return item;
        });
        this.refreshItems = function (input, url, append) {
          // If they typed in a search, reset skip.
          if ((_this.lastInput || input) && _this.lastInput !== input) {
          ...
          if (input) {
            items = items.filter(function (item) {
              switch (_this.props.component.filter) {
                case 'startsWith':
        ...
        break;
      case 'custom':
        this.refreshItems = function (input) {
          try {
            ...
            var selectItems = eval('(function(data, row, input) { var values = [];' + _this.props.component.data.custom.toString() + '; return values; })(data, row, input)');
            ...
            console.warn('Error calculating select components in ' + _this.props.component.key, error);
            _this.setState({
              selectItems: []
            });
          }
      ...
      case 'url':
        if (this.props.component.dataSrc === 'url') {
          ...
          this.props.component.data.headers.forEach(function (header) {
            _this.options.headers.set(header.key, header.value);
          });
        } else {
