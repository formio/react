...
exports.default = function (authSettings) {
  return function (_ReduxView) {
    ...
    function _class2() {
      ...
      return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = _class2.__proto__ || Object.getPrototypeOf(_class2)).call.apply(_ref, [this].concat(args))), _this), _this.container = function (_ref2) {
        ...
        var init = store.getState().get('formio').get('auth').get('init');
        if (!init) {
        ...
        var auth = state.get('formio').get('auth');
        console.log(auth, authSettings);
        return {
          shouldRedirect: authSettings.forceAuth && authSettings.allowedStates.length && auth.init && !auth.isFetching && !auth.user && !authSettings.allowedStates.includes(location.pathname),
