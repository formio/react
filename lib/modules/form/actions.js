...
var getForm = exports.getForm = function getForm(name) {
  ...
  return function (dispatch, getState) {
    ...
    var _selectRoot = (0, _root.selectRoot)(name, getState()),
        options = _selectRoot.options,
        projectUrl = _selectRoot.projectUrl;

    if (form.components && Array.isArray(form.components) && form.components.length && form._id === id) {
      return;
    }

    var path = (projectUrl || _Formio2.default.getProjectUrl()) + '/' + (id ? 'form/' + id : name);
    var formio = new _Formio2.default(path);

    dispatch(requestForm(name, id, path));

    return formio.loadForm({}, options).then(function (result) {
...
var deleteForm = exports.deleteForm = function deleteForm(name) {
  var done = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : function () {};
  return function (dispatch, getState) {
    var form = (0, _selectors.selectForm)(name, getState());
    var path = _Formio2.default.getProjectUrl() + '/form/' + form._id;
    var formio = new _Formio2.default(path);
