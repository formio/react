...
import { textField, visible, layout, columns, formWithInput } from './fixtures';

import { createIfExceed, createMount, seq } from './utils';
...
describe('Form component', function () {
    ...
    it('should create formio instance.', function () {
        ...
        return element.instance().createPromise.then((formio) => {
        ...
        return seq([
            () => {
                element = mount(
                    <Form
                        ...
                        onChange={onchange}
                        submission={{
                            data: { visible: false, textfield: 'Test' },
                        }}
                    />,
                );
                return ifexceed('fail on init change', () => {
                    ...
                    root = element.getDOMNode();
                    expect(onchange.calledOnce).to.be.true;
                    expect(formio.submission).to.have.deep.property('data', {
                        visible: false,
                    });
                });
            },
            () => {
                ...
                return ifexceed('fail on checkbox click', () => {
                    expect(onchange.calledTwice).to.be.true;
                    expect(formio.submission).to.deep.equal({
                        data: { visible: true, textfield: '' },
                    });
                });
            },
            () => {
                ...
                return ifexceed('fail on textfield input', () => {
                    expect(onchange.callCount).to.equal(3);
                    expect(formio.submission).to.deep.equal({
                        data: { visible: true, textfield: 'value' },
                    });
                });
            },
            () => {
            ...
            () => {
                element = mount(
                    <Form
                        ...
                        onChange={onchange}
                        submission={{
                            data: { visible: true, textfield: 'Test' },
                        }}
                    />,
                );
                return ifexceed('fail on init change', () => {
    ...
    it('should have own event emitter', function () {
        ...
        return seq([
            () => {
                ...
                return Promise.all([
                    timeout1('Form1 fail on init change', () => {
                        expect(
                            onchange1.calledOnce,
                            'Form1 init change triggerd more then once',
                        ).to.be.true;
                    }),
                    timeout2('Form2 fail on init change', () => {
                        expect(
                            onchange2.calledOnce,
                            'Form2 init change triggerd more then once',
                        ).to.be.true;
                    }),
                ]);
            },
            () => {
                ...
                return timeout1('input1 change is not triggered', () => {
                    expect(
                        ...
                        'Form1#onChange triggerd more then twice after input1 change',
                    ).to.be.true;
                });
            },
            () => {
                expect(onchange2.calledOnce, 'input1 triggerd Form2#onChange')
                    .to.be.true;
            ...
            () => {
                expect(onchange1.calledTwice, 'input2 triggered Form1#onChange')
                    .to.be.true;
            },
        ]);
    });
