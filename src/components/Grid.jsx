	...
	const getColumn = (column) => {
		...
		return (
			<div {...columnProps}>
				...
				>
					<strong>
						{title} <span className={sortClass} />
					</strong>
				</span>
			</div>
		);
	...
	const PageSizeSelector = () => (
		<div className="col-auto">
			<div className="row align-items-center">
				...
				<span className="col-auto">items per page</span>
			</div>
		</div>
	);
	...
	const Footer = () => (
		<li className="list-group-item">
			<div className="row align-items-center">
				...
				{renderItemCounter(props) ? <ItemCounter></ItemCounter> : null}
			</div>
		</li>
	);
			...
			{items.length ? (
				<ul className="list-group list-group-striped">
					<li className="list-group-item list-group-header hidden-xs hidden-md">
						<div className="row">{columns.map(getColumn)}</div>
					</li>
					{items.map(getItem)}
					{renderFooter(props) ? <Footer></Footer> : null}
				</ul>
			) : (
				<div>{emptyText}</div>
			)}
